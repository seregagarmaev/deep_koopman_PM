...
from models import DeepKoopmanExperiment2
from config import *
from utils import *
...
for (train_machine1, train_machine2), test_machine in zip(train_machines, test_machines):
    if data_processed:
        ...
        joblib.dump(std_scaler, f'models/experiment_2/std_scaler_{train_machine1}{train_machine2}.save')
        joblib.dump(minmax_scaler, f'models/experiment_2/minmax_scaler_{train_machine1}{train_machine2}.save')
    ...
    training_loader = DataLoader(dataset, batch_size=model_params_exp2['bs'], shuffle=True, drop_last=True)


    print('Initializing a model')
    model = DeepKoopmanExperiment2(model_params_exp2)
    optimizer = torch.optim.Adam(model.parameters(), lr=model_params_exp2['lr'], weight_decay=1e-7)


    print('Training the model')
    for epoch in range(1, model_params_exp2['epochs']+1):
        train_loss, (supervised_loss, prediction_loss) = train_model_exp2(model, optimizer, training_loader)
        print(f'Epoch {epoch}. Train loss {train_loss}')
        print(f'Supervised loss {supervised_loss}, Prediction loss {prediction_loss}.')
    ...
    for obs_i in range(model_params_exp2['obsdim']):
    ...
    plt.legend()
    plt.xlabel('step')
    plt.savefig(f'plots/experiment_2/observables_RUL_machine_{test_machine}.png')
    plt.close()
    ...
    plt.legend()
    # plt.yscale('log')
    plt.savefig(f'plots/experiment_2/RUL_modelling_inner_model_machine_{test_machine}.png')
    plt.close()
    ...
    plt.legend()
    # plt.yscale('log')
    plt.savefig(f'plots/experiment_2/custom_metric_machine_{test_machine}.png')
    plt.close()
...
rmse.to_csv('models/experiment_2/rmse.csv')

joblib.dump(mean_rul_prediction_error, 'models/experiment_2/mean_custom_metric.txt')
joblib.dump(rul_prediction_error, 'models/experiment_2/custom_metric.txt')