...
# The results are summed and passed to the linear Koopman operator.
...
cnc_dataset = CNC_dataset(f'../DATASETS/cnc_milling_machine/c{4}', machine=4, columns=data_columns, use_RUL=True, wear_threshold=wear_threshold)
...
processed_data.to_feather(f'../DATASETS/cnc_milling_machine/processed_data_machines_{1}{4}.feather')
...
for epoch in range(1, control_model_params['epochs']+1):
    train_loss, (final_pred_loss, final_autoencoder_loss, final_supervised_loss) = train_model_2(model, optimizer, training_loader)
...
print('Evaluation')
model.eval()
cnc_dataset = CNC_dataset(f'../DATASETS/cnc_milling_machine/c{1}', machine=1, columns=data_columns, use_RUL=True, wear_threshold=wear_threshold)
init_data = cnc_dataset.get_data()
